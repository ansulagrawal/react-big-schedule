...
export default class SchedulerData {
  ...
  setVersionChangeNotifier(callback) {
  ...
  getSchedulerHeight() {
    ...
    const totalHeaderHeight =
      this.config.tableHeaderHeight + (this.config.showWeekNumber ? this.config.weekNumberRowHeight || 24 : 0);
    const baseHeight = this.documentHeight - headerHeight - totalHeaderHeight - (this.config.underneathHeight || 0);
  ...
  _resolveDate(num, date = undefined) {
    ...
    switch (this.viewType) {
      ...
      case ViewType.VerticalResource:
        this.startDate = date !== undefined ? this.selectDate.startOf('day') : this.startDate.add(num, 'days');
        this.endDate = this.startDate;
        break;

      case ViewType.Week:
        setStartAndEndDates('isoWeek');
        break;
    ...
    if (this.showAgenda) {
      ...
      while (header >= start && header <= end) {
        ...
        for (let i = 0; i < minuteSteps; i += 1) {
          ...
          if (hour >= this.config.dayStartFrom && hour <= this.config.dayStopTo) {
            ...
            timeSlots.push({ time, nonWorkingTime });
          }
      ...
      while (header >= start && header <= end) {
        ...
        if (this.config.displayWeekend || (dayOfWeek !== 0 && dayOfWeek !== 6)) {
          const nonWorkingTime = this.behaviors.isNonWorkingTimeFunc(this, time);
          timeSlots.push({ time, nonWorkingTime, style });
        }
  ...
  _createInitRenderData(isEventPerspective, eventGroups, resources, headers) {
    const slots = this.isVerticalResourceView()
      ? this.timeSlots.map(ts => ({
          ...ts,
          id: ts.time,
          name: ts.time,
          slotId: ts.time,
